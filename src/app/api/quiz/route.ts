...
export async function POST(request: Request) {
  ...
  const questions = body.questions.map((q: { text: string; options: unknown[]; correctIndex: number; image?: { data?: string; contentType?: string; width?: number; height?: number } }) => {
    const image = q.image;
    ...
    if (options.length !== 4 || options.some((option) => !option) || !Number.isInteger(Number(q.correctIndex)) || Number(q.correctIndex) < 0 || Number(q.correctIndex) >= options.length) throw new Error("Each question needs four valid options and a correct answer.");
    ...
    return { text: String(q.text ?? "").trim(), options, correctIndex: Number(q.correctIndex), image: base64 ? { bytes: Buffer.from(base64, "base64"), contentType: image?.contentType, width: image?.width, height: image?.height } : undefined };
  });
