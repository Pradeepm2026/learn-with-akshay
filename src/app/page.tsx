...
export default function Home() {
  ...
  const [liveReportRows, setLiveReportRows] = useState<Array<{ name: string; correct: number; incorrect: number; total: number }>>([]);
  ...
  const loadLiveData = async () => {
    ...
    setPublishedQuiz(quiz ? { id: quiz.id, title: quiz.title, subject: quiz.subject, timeLimit: quiz.timeLimit ?? "", availableOn: quiz.availableOn } : null); setQuizQuestions(quiz?.questions.map((question) => ({ question: question.text, options: question.options, correct: question.options[question.correctIndex], imageUrl: question.imageUrl ?? undefined })) ?? []);
    ...
    setAssignmentSubmissions(data.submissions as AssignmentSubmission[] ?? []); setLeaderboard(data.leaderboard as Array<{ rank: number; name: string; correct: number; total: number }> ?? []); if (data.stats) setLiveAdminStats(data.stats as { studentCount: number; attempts: number; averageScore: number | null }); if (data.reportRows) setLiveReportRows(data.reportRows as Array<{ name: string; correct: number; incorrect: number; total: number }>);
  };
  ...
  const saveAndContinue = () => {
    ...
    const nextAnswers = quizAnswers.map((answer, index) => index === currentQuestion ? selectedAnswer : answer);
  ...
  const submitQuiz = async () => {
    const finalAnswers = quizAnswers.map((answer, index) => index === currentQuestion ? selectedAnswer : answer);
    if (!publishedQuiz) return;
    try { const answers = finalAnswers.map((answer, index) => quizQuestions[index].options.indexOf(answer)); const response = await fetch(`/api/quiz/${publishedQuiz.id}/submit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ answers }) }); const data = await readResponse<{ error?: string; correct: number; incorrect: number; attemptCount: number }>(response); if (!response.ok) throw new Error(data.error ?? "Could not submit quiz."); setQuizAnswers(finalAnswers); setQuizScore({ correct: data.correct, incorrect: data.incorrect }); setQuizAttemptsUsed(data.attemptCount); setSubmitted(true); setTab("My Results"); await loadLiveData(); } catch (error) { setUploadError(error instanceof Error ? error.message : "Could not submit quiz."); }
  };
  const startNextQuizAttempt = () => { autoSubmittingQuizRef.current = false; setQuizDeadline(null); setQuizSecondsLeft(null); setQuizAnswers(Array(quizQuestions.length).fill("")); setCurrentQuestion(0); setDraftAnswer(""); setQuizScore({ correct: 0, incorrect: 0 }); setSubmitted(false); setTab("Daily Quiz"); };
  useEffect(() => {
      ...
      <section className="content">
        ...
        {role === "admin" && tab === "Daily Quiz" && <article className="workspace admin-workspace">
          ...
          <section className="question-builder">
            ...
            <div className="option-grid">{["A", "B", "C", "D"].map((letter, index) => <label key={letter}>{letter}<input value={adminOptions[index]} onChange={(event) => setAdminOptions((options) => options.map((option, optionIndex) => optionIndex === index ? event.target.value : option))} placeholder={`Option ${letter}`} /></label>)}</div>
        ...
        {role === "admin" && tab === "Quiz Reports" && (isQuizLive && publishedQuiz ? <><div className="section-title report-title"><div><p>DAILY QUIZ · {dateCard.day} {dateCard.monthYear.toUpperCase()}</p><h2>{publishedQuiz.title}</h2><span>{liveReportRows.length} student{liveReportRows.length === 1 ? "" : "s"} attempted this quiz.</span></div>{liveReportRows.length > 0 && <button className="dark-button" onClick={() => window.print()}>Print live report</button>}</div>{liveReportRows.length ? <div className="table"><div className="table-head"><span>Student</span><span>Correct</span><span>Incorrect</span><span>Score</span></div>{liveReportRows.map((row) => <div className="table-row" key={row.name}><span>{row.name}</span><span>{row.correct} / {row.total}</span><span>{row.incorrect}</span><span>{Math.round((row.correct / row.total) * 100)}%</span></div>)}</div> : <p className="empty-notes">No student has submitted this quiz yet.</p>}</> : <p className="empty-notes">Publish today&apos;s quiz to view its live report.</p>)}
      </section>
    </div>
